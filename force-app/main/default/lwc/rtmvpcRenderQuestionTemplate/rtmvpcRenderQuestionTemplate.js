...
export default class RtmvpcRenderQuestionTemplate extends LightningElement {
    ...
    /* openChatHandler is used to dispatch an event to its parent component(Questionnaire) by sending the questionId to parent component to open chat conversation */
    ...
    uploadFilesHandler(event) {
        ...
        console.log('event.currentTarget.dataset.Id;', event.target.id);
    ...
    removehighlightHandler(id) {
        if (typeof id != 'undefined') {
            var labelsList = this.template.querySelectorAll('.qactivelabelcont');
            for (var i = 0; i < labelsList.length; i++) {
                console.log('labelsList[i].dataset.key' + labelsList[i].dataset.id + ' : ' + id);
